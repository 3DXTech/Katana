...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    ...
    { EProducer::KatanaSlicer, "generated by KatanaSlicer"}
};
...
void GCodeProcessor::apply_config(const PrintConfig& config)
{
    ...
    m_producer = EProducer::KatanaSlicer;
    ...
    m_extruder_last_toolchange.resize(extruders_count);
    ...
    for (size_t i = 0; i < extruders_count; ++ i) {
        ...
        m_extruder_standbytemps_config[i]      = static_cast<int>(config.idle_temperature.get_at(i));
...
void GCodeProcessor::apply_config(const DynamicPrintConfig& config)
{
    ...
    m_extruder_last_toolchange.resize(m_result.extruders_count);
...
void GCodeProcessor::reset()
{
    ...
    m_extruder_last_toolchange.resize(MIN_EXTRUDERS_COUNT);
    for (size_t i = 0; i < MIN_EXTRUDERS_COUNT; ++i) {
        m_extruder_last_toolchange[i] = NULL;
    }
...
void GCodeProcessor::process_ascii_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        if (m_producer == EProducer::PrusaSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r || m_producer == EProducer::KatanaSlicer) {
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::PrusaSlicer || m_producer == EProducer::KatanaSlicer) {
        // height tag
...
void GCodeProcessor::process_T(const std::string_view command)
{
    if (command.length() > 1) {
        ...
        if (! parse_number(command.substr(1), eid) || eid < 0 || eid > 255) {
            ...
            if (m_extruder_id != id) {
                if (((m_producer == EProducer::PrusaSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r || m_producer == EProducer::KatanaSlicer) && id >= m_result.extruders_count) ||
                    ((m_producer != EProducer::PrusaSlicer && m_producer != EProducer::Slic3rPE && m_producer != EProducer::Slic3r && m_producer != EProducer::KatanaSlicer) && id >= m_result.extruder_colors.size()))
...
void GCodeProcessor::post_process()
{
    ...
    public:
        ...
        void insert_preheat_lines(const Backtrace &                                      backtrace,
                                  ...
                                  const double                                           last_toolchange_time,
                                  const double                                           heat_rate,
                                  const double                                           cooldown_rate)
        {
            assert(!m_lines.empty());
            size_t rev_it_dist         = 0;    // distance from the end of the cache of the starting point of the backtrace
            float  last_time_insertion = 0.0f; // used to avoid inserting two lines at the same time

            double predicted_temp     = active_temp - ((m_lines.back().time - last_toolchange_time) / cooldown_rate);
            double standby_temp_delta = active_temp - standby_temp;
            // (temperature delta / heat rate) * time compensation to handle heat acceleration
            const float backtrace_time_i = predicted_temp <= active_temp - standby_temp_delta ? (standby_temp_delta / heat_rate) * 1.5 : ((active_temp - predicted_temp) / heat_rate) * 1.5;
            const float time_threshold_i = m_time - backtrace_time_i;
            auto        rev_it           = m_lines.rbegin() + rev_it_dist;
            auto        start_rev_it     = rev_it;
        ...
        void write(FilePtr& out, float backtrace_time, GCodeProcessorResult& result, const std::string& out_path) {
            ...
            if (!m_lines.empty()) {
                if (m_write_type == EWriteType::ByTime) {
                    ...
                    if (m_size > 65535) {
                        while (!m_lines.empty()) {
                            std::string line = m_lines.front().line;
            ...
            if (tool_number != -1) {
                ...
                if (backtraceEnabled) {
                    ...
                    export_lines.insert_preheat_lines(
                        ...
                        m_extruder_last_toolchange[tool_number],
