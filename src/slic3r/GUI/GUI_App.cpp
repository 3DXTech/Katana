...
//        scale_bitmap(m_main_bitmap, m_scale);
    ...
    void SetText(const wxString& text)
    {
        ...
        if (!text.empty()) {
            ...
            memDC.SetTextForeground(wxColour(183, 49, 44));
...
{
    return dark_mode() ? wxColour(183, 49, 44) : wxColour(183, 49, 44);
}
...
void GUI_App::init_ui_colours()
{
    ...
    m_color_hovered_btn_label       = is_dark_mode ? wxColour(183, 49, 44) : wxColour(202, 36, 29);
    m_color_default_btn_label       = is_dark_mode ? wxColour(247, 87, 87): wxColour(203, 0, 0);
...
void GUI_App::open_web_page_localized(const std::string &http_address, bool addLang)
{
    open_browser_with_warning_dialog(http_address + (addLang ? ("&lng=" + this->current_language_code_safe()) : ""), nullptr, false);
}
...
bool GUI_App::config_wizard_startup()
{
    if (!m_app_conf_exists || preset_bundle->printers.only_default_printers()) {
        ...
        return true;
    } else if (false/*get_app_config()->legacy_datadir()*/) // Always returning false for now to disable config wizard on every startup
    {
